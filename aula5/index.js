const dados = [
  {
    ...
    notaGA: 5.7,
    notaGB: 6.2,
    media: function () { return (this.notaGA + this.notaGB) / 2 }
  },
  {
...
const montaGrid = () => {
  ...
  dados.forEach(el => {
    ...
    const aprovadoEl = document.createElement('td');
    aprovadoEl.className = el.media() >= 6 ? 'table-success' : 'table-danger';
    rowEl.append(aprovadoEl);
  ...
  const mediaTurma = dados
    ...
    resultados.insertAdjacentHTML('beforeend', `<h4>Média da turma: ${ mediaTurma }</h4><h3>Melhor Aluno: ${melhorAluno.nome} nota: ${melhorAluno.media()}</h4>`);
